...
from plotly.subplots import make_subplots # เพิ่มสำหรับสร้างกราฟ 2 แกน
import re
import io
...
st.markdown("""
    ...
    h2, h3 { color: #374151; font-weight: 500; }
...
def get_target_sheet(xls):
    ...
    for sheet in sheet_names:
        match = re.search(r'rev\.?\s*(\d+)', sheet, re.IGNORECASE)
        if match:
            rev_sheets[sheet] = int(match.group(1))
    if rev_sheets:
        return max(rev_sheets, key=rev_sheets.get)
    for sheet in sheet_names:
        if 'ประมาณการ' in sheet:
            return sheet
...
def process_data(plan_files, actual_file):
    ...
    for file in plan_files:
        ...
        cleaned_headers = [h.strftime('%Y-%m-%d') if isinstance(h, pd.Timestamp) else h.split(' ')[0] if isinstance(h, str) and '00:00:00' in h else str(h) for h in headers]
        df = df.iloc[2:].copy()
...
with st.sidebar:
    ...
    plan_files = st.file_uploader("1. อัปโหลดไฟล์แผน (Plan)", accept_multiple_files=True, type=['xlsx'], help="เลือกไฟล์ Excel แผนงานได้หลายไฟล์พร้อมกัน")
    actual_file = st.file_uploader("2. อัปโหลดไฟล์รับเข้า (Actual)", type=['xlsx'], help="ไฟล์ Copy of export.xlsx")
    st.markdown("---")

# ================= 4. ส่วนแสดงผลหลัก (Main UI) =================
if plan_files and actual_file:
    with st.spinner("⏳ กำลังประมวลผลข้อมูลระดับโรงงาน..."):
        df_merged = process_data(plan_files, actual_file)
    ...
    if not df_merged.empty:
        ...
        with tab1:
            ...
            # สร้างกราฟ 2 แกน (Dual Axis)
            fig = make_subplots(specs=[[{"secondary_y": True}]])
            
            # เพิ่มแท่งแผน (Plan) บนแกนซ้าย
            fig.add_trace(
                go.Bar(x=df_sub['Subcontractor'], y=df_sub['Plan_Qty'], name='Plan (แผน)', marker_color='#1E3A8A'),
                secondary_y=False,
            )
            # เพิ่มแท่งรับจริง (Actual) บนแกนซ้าย
            fig.add_trace(
                go.Bar(x=df_sub['Subcontractor'], y=df_sub['Actual_Qty'], name='Actual (รับจริง)', marker_color='#10B981'),
                secondary_y=False,
            )
            # เพิ่มเส้น % ความสำเร็จ (Achievement) บนแกนขวา
            fig.add_trace(
                go.Scatter(x=df_sub['Subcontractor'], y=df_sub['Achievement %'], name='Achievement %',
                           mode='lines+markers+text', 
                           text=df_sub['Achievement %'].apply(lambda x: f"{x:.1f}%"),
                           textposition="top center",
                           line=dict(color='#F59E0B', width=3), # สีส้มทอง
        ...
        with tab3:
            ...
            st.download_button(
                label="📥 ดาวน์โหลดข้อมูล (Export to Excel)",
                data=buffer.getvalue(),
                file_name="Subcontractor_Detailed_Data.xlsx",
    ...
    st.info("👈 กรุณาอัปโหลดไฟล์แผน (Plan) และไฟล์รับเข้า (Actual) ที่แถบเมนูด้านซ้ายเพื่อเริ่มต้นใช้งานระบบ")